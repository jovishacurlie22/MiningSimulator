/*
 * ============================================================================
 * Blockchain-Inspired Mining Simulator — Revamped Edition
 * ============================================================================
 *
 * Original bugs fixed:
 *  1. Variable scope: 'input' re-declared in different switch cases (undefined
 *     behaviour / compile error on strict compilers). Fixed: all locals
 *     declared at function top before the switch.
 *
 *  2. Nested mutex deadlock: ncurses_mutex was acquired WHILE block_mutex was
 *     held. If UI thread locked them in reverse order → deadlock. Fixed: 
 *     log messages go into a thread-safe ring buffer; a dedicated display
 *     thread drains it, holding only ncurses_mutex.
 *
 *  3. Priority inversion: sem_wait(&ledger_sem) called while holding
 *     block_mutex. Fixed: semaphore removed; ledger protected by block_mutex
 *     already held at that point.
 *
 *  4. Broken blockchain: each miner had its own local prev_hash, so different
 *     miners diverged into separate chains. Fixed: single global prev_hash
 *     protected by ctrl_mutex, updated atomically when a block is accepted.
 *
 *  5. Integer overflow in meets_difficulty: "(1UL << (difficulty*4)) - 1"
 *     overflows for difficulty >= 16. Fixed: iterative nibble-check loop
 *     that shifts by 4 bits at a time.
 *
 *  6. Exit race: mining threads could be stuck in pthread_cond_wait after UI
 *     returned and called endwin(). Fixed: global 'terminate' flag plus final
 *     broadcast-and-join sequence.
 *
 *  7. Stale reads of shared globals without lock in miner loop (simulation_
 *     running, block_count) without holding the mutex. Fixed: snapshot shared
 *     state under mutex before use, rechecked with lock when needed.
 *
 *  8. getch() called without ncurses_mutex: race with wprintw in display
 *     thread. Fixed: ncurses_mutex held around every ncurses call.
 *
 * BUILD:
 *   gcc -Wall -Wextra -O2 -o mining_sim mining_sim_final.c -lpthread -lncurses
 *
 * RUN:
 *   ./mining_sim
 * ============================================================================
